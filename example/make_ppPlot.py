...
def get_all_quantile(filename):
    ...
    print(data.keys())
    chains = data['chains']
    true_param = data['true_params']
    #true_param[1] = q_interp(true_param[1])
    #true_param[7] = np.arccos(np.arcsin(np.sin(true_param[7]/2*np.pi))*2/np.pi)
    #true_param[10] = np.arcsin(np.arcsin(np.sin(true_param[10]/2*np.pi))*2/np.pi)
    
    # This transforms cos_iota and sin_dec recorded in chains into iota and dec
    ...
    for i in range(11):
        print(i)
        result.append(compute_percentile_multimodal(true_param[i],chains[:,:,i].reshape(-1)[::10]))
...
np.savez('/home/zwang264/scr4_berti/zipeng/PPE_with_Jax/jim/example/zw_test_batch_out/combined_quantile_balance_LVK',result=result, result_multimodal=result_multimodal, true_param=true_param, mean_global_accs=mean_global_accs, mean_local_accs= mean_local_accs)

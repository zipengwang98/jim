...
parser.add_argument(
    "--downsample_factor", type=int, default=1, help="downsample factor"
)
...
def gen_waveform_H1(f, theta):
    ...
    return H1_response(f, hp, hc, ra, dec, gmst, theta[8]) * jnp.exp(
        -1j * 2 * jnp.pi * f * (epoch + theta[5])
    )
...
def gen_waveform_L1(f, theta):
    ...
    dec = theta[10]
    hp, hc = gen_IMRPhenomD_polar(f, theta_waveform, f_ref)
    return L1_response(f, hp, hc, ra, dec, gmst, theta[8]) * jnp.exp(
        -1j * 2 * jnp.pi * f * (epoch + theta[5])
    )
...
logL = make_heterodyne_likelihood_mutliple_detector(
    data_list,
    psd_list,
    response_list,
    gen_IMRPhenomD_polar,
    ref_param,
    f_list,
    gmst,
    epoch,
...
guess_param = np.array(
    jnp.repeat(true_param[None, :], int(n_chains), axis=0)
    * (
        1
        + 0.1
        * jax.random.normal(
            jax.random.PRNGKey(seed + 98127), shape=(int(n_chains), n_dim)
        )
    )
)
...
for i in range(n_dim):
    initial_position = initial_position.at[:, i].set(
        initial_position[:, i] * (prior_range[i, 1] - prior_range[i, 0])
        + prior_range[i, 0]
    )
...
def posterior(theta):
    ...
    jacobian = jnp.log(
        (1 / (1 + q) ** 2) - 2 * q / (1 + q) ** 3
    )  # - jnp.log(jnp.sin(iota)) - jnp.log(jnp.sin(dec))
...
np.savez(
    ...
    local_accs=local_accs[:, ::downsample_factor],
    global_accs=global_accs[:, ::downsample_factor],
    loss_vals=loss_vals,
    labels=labels,

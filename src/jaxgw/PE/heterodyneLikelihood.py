...
def make_heterodyne_likelihood(data, h_function, ref_theta, psd, freqs, n_bins=101):
    ...
    A0, A1, B0, B1 = compute_coefficients(
        data, h_ref, psd, freqs, f_bins, f_bins_center
    )
...
def make_heterodyne_likelihood_mutliple_detector(
    data_list,
    psd_list,
    respose_list,
    h_function,
    ...
    for i in range(num_detector):
        h_ref.append(
            respose_list[i](freqs, raw_hp, raw_hc, ra, dec, gmst, ref_theta[8])
